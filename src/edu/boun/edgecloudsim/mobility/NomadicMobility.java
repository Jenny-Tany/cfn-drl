...
public class NomadicMobility extends MobilityModel {
	private boolean devicesMobile= SimSettings.devicesMobile;
	...
	@Override
	public void initialize(topoGraph topo) {
		treeMapArray = new ArrayList<TreeMap<Double, Location>>();//时间戳，位置
		
		ExponentialDistribution[] expRngList = new ExponentialDistribution[SimSettings.getInstance().getNumOfEdgeDatacenters()];

		//create random number generator for each place为每个数据中心创建一个指数分布的随机数生成器
		...
		for(int i=0; i<numberOfMobileDevices; i++) {
			...
			if(devicesMobile){
				...
				int placeTypeIndex = Integer.parseInt(attractiveness);//吸引度
				...
				int randId;
				if(i<numberOfMobileDevices/2)  randId = SimUtils.getRandomNumber(0,topo.getNodeCount()-1);
				else {
					randId = SimUtils.getRandomNumber(0,topo.getHotNode().length-1);
					randId = topo.getHotNode()[randId];
				}
				treeMapArray.get(i).put(SimSettings.CLIENT_ACTIVITY_START_TIME, new Location(0, randId, randId, randId));
			}
