...
public class SimSettings {
	...
	public static boolean devicesMobile=false;
    ...
    private int ACTION_SIZE,LOADSIZE;
...
//			STORAGE_FOR_CLOUD_VM = Integer.parseInt(prop.getProperty("storage_for_cloud_vm"));
			...
			ACTION_SIZE = Integer.parseInt(prop.getProperty("action_size"));
			LEARNINGRATE = Double.parseDouble(prop.getProperty("learning_rate"));
			TOPO = prop.getProperty("topo");
			OutputFolder = prop.getProperty("output_folder");
			LOADSIZE = Integer.parseInt(prop.getProperty("load_data_size"));
	...
	private void parseApplicatinosXML(String filePath)
	{
		...
		try {	
			...
			for (int i = 0; i < appList.getLength(); i++) {
			    ...
			    taskLookUpTable[i][0] = usage_percentage; //usage percentage [0-100]任务相对出现率，所有种类任务的出现率和为1
			    ...
			    taskLookUpTable[i][5] = data_upload*LOADSIZE; //avg data upload (KB)
			    taskLookUpTable[i][6] = data_download*LOADSIZE; //avg data download (KB)
