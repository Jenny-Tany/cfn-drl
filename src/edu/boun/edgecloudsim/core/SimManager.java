...
public class SimManager extends SimEntity {
	...
	private double reward=0,lastreward=0;
	...
	public SimManager(ScenarioFactory _scenarioFactory, int _numOfMobileDevice, String _simScenario, String _orchestratorPolicy) throws Exception {
		...
		mobilityModel.initialize(networkModel.getTopo());
		SimLogger.printLine("Done.");

		//Generate edge orchestrator初始化适配器
...
//		cloudServerManager.initialize();
	...
	public void startSimulation() throws Exception{
		...
		//Start cloud Datacenters & Generate VMs  we dont need
...
//		}
	...
	public void processEvent(SimEvent ev) {
		synchronized(this){//只有一个线程可以执行这段代码，确保线程安全
			switch (ev.getTag()) {
			case CREATE_TASK:
				try {
			...
			case STOP_SIMULATION:
				...
				SimLogger.getInstance().addVmUtilizationLog(//再记录
						CloudSim.clock(),
						edgeServerManager.getAvgUtilization(),
						networkModel.getAvgBandwidth(),
						reward-lastreward
