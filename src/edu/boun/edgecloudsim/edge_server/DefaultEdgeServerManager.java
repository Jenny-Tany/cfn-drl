...
public class DefaultEdgeServerManager extends EdgeServerManager{
	...
	public void createVmList(int brockerId){
		...
		Document doc = SimSettings.getInstance().getEdgeDevicesDocument();
		NodeList datacenterList = doc.getElementsByTagName("datacenter");
		for (int i = 0; i < datacenterList.getLength(); i++) {//遍历14个数据中心
			Node datacenterNode = datacenterList.item(i);
			Element datacenterElement = (Element) datacenterNode;
			NodeList hostNodeList = datacenterElement.getElementsByTagName("host");
			for (int j = 0; j < hostNodeList.getLength(); j++) {//遍历1个主机
				...
				for (int k = 0; k < vmNodeList.getLength(); k++) {//遍历八个虚拟机
					Node vmNode = vmNodeList.item(k);					
					Element vmElement = (Element) vmNode;
//读取参数，虚拟化管理器（vmm）、核心数（numOfCores）、MIPS 值（mips）、内存（ram）、存储（storage）
...
//					WlanBandwidth 除以（主机列表长度+虚拟机列表长度）
