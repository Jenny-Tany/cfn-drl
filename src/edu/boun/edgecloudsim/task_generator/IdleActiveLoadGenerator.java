...
public class IdleActiveLoadGenerator extends LoadGeneratorModel{
	...
	public void initializeModel() {
		...
		for(int i=0; i<SimSettings.getInstance().getTaskLookUpTable().length; i++) {
			...
			expRngList[i][0] = new ExponentialDistribution(SimSettings.getInstance().getTaskLookUpTable()[i][5]);//上传大小
			expRngList[i][1] = new ExponentialDistribution(SimSettings.getInstance().getTaskLookUpTable()[i][6]);//下载大小
			expRngList[i][2] = new ExponentialDistribution(SimSettings.getInstance().getTaskLookUpTable()[i][7]);//任务长度
		}
		...
		for(int i=0; i<numberOfMobileDevices; i++) {
			...
			while(virtualTime < simulationTime) {
				...
				taskList.add(new TaskProperty(i,randomTaskType, virtualTime, expRngList));//第三个参数为开始时间
			}
		}
