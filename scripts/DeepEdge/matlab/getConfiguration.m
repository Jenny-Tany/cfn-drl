...
function [ret_val] = getConfiguration(argType)
    if(argType == 1)
        ret_val = '../../../sim_results';
    elseif(argType == 2)
        ret_val = 40; %Number of iterations
    elseif(argType == 3)
        ret_val = 0; %min number of mobile device
    elseif(argType == 4)
    ...
    elseif(argType == 7)
        ret_val = {'DDQN','FUZZY_BASED','NETWORK_BASED','UTILIZATION_BASED','HYBRID'};
        %ret_val = {'DDQN','FUZZY_BASED'};
    elseif(argType == 8)
        ret_val = {'DeepEdge', 'Fuzzy Based','Network Based', 'Utilization Based', 'Hybrid'};
        %ret_val = {'DeepEdge', 'Fuzzy Based'};
    elseif(argType == 9)
    ...
    elseif(argType == 51) % Baris func
        ret_val = {'DEEP_EDGE_VM'}; % for specific calculations
    end
end